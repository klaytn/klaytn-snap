...
export { MetamaskKlaytnSnap } from "./snap";
export {
  hasMetaMask,
  isMetamaskSnapsSupported,
  isSnapInstalled,
...
/**
 * Install and enable Klaytn snap
 *
 ...
 * @param snapOrigin
 *
 * @return MetamaskKlaytnSnap - adapter object that exposes snap API
 */
export async function enableKlaytnSnap(
  config: Partial<SnapConfig>,
  snapOrigin?: string,
  snapInstallationParams: Record<SnapInstallationParamNames, unknown> = {}
): Promise<MetamaskKlaytnSnap> {
  ...
  const snap = new MetamaskKlaytnSnap(snapOrigin || defaultSnapOrigin);
  // set initial configuration
  await (await snap.getKlaytnSnapApi()).configure(config);
  // return snap object
  return snap;
}
